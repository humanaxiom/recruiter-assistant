...
| **Department & campus** (sponsor, 2026-09-03) | **Done** — `jd_extract_v2` extracts both; `record_parsed` writes them onto the row (fill-when-empty, so an override survives a re-parse); `src/campus.py` canonicalises Burnaby/BBY · Vancouver/YVR · Surrey/SRY; a form on the job page fills or overrides either. `scripts/backfill_job_fields.py` reached the 25 existing rows. **Departments still need a re-parse**: 20 jobs, unblocked now that the timeout is corrected on this box, ~4 min of LLM each. `--reparse-plan` prints the ids. |
