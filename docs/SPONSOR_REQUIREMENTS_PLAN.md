...
- The CSV carries PII — and the property it has is **stronger** than the encryption boundary: **none of the roster is persisted at all**. It is parsed in memory, matched, and discarded; only the three screening columns on `resumes` outlive the request. Never embedded (ADR-008). The reconciliation report carries counts and line numbers only — never a decrypted name or email, and neither does its audit event.
