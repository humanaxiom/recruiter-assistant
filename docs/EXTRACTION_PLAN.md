...
| **4 · Ranking engine** | Split into 4 gated sub-phases (below) — each its own branch/PR, `make gates` + reviewer/security/ranking-evals green before the next | ✅ done — all 4 sub-phases merged (started 2026-07-12, last merged 2026-07-16) |
...
labelled fixtures + matched-pair dimension controls + `thresholds.toml` + a RED-pending-4c harness stub —
see [activity/phase-4a-ranking-evals-corpus.md](activity/phase-4a-ranking-evals-corpus.md)). Its
...
BLOCKERS" below (now CLOSED). **Sub-phase 4c (matching engine) is COMPLETE and MERGED to `main` via
PR #12** (https://github.com/humanaxiom/recruiter-assistant/pull/12), merge `fd12d1a`, CI green. See
[activity/phase-4c-matching-engine.md](activity/phase-4c-matching-engine.md) and
[ADR-009](adr/009-matching-engine-port.md). **Sub-phase 4d (shortlist + reverse-match write path) is
COMPLETE and MERGED to `main` via PR #13** (merge `5945320`, CI green, merged 2026-07-16), tip `6c2bf43`,
2 commits, off `main` @ `fd12d1a`. See
[activity/phase-4d-shortlist-writepath.md](activity/phase-4d-shortlist-writepath.md) and
[ADR-010](adr/010-shortlist-reverse-match-write-path.md). **Phase 5 (persist + anonymize + export) is
MERGED to `main` via PR #14** (merge `6deade3`, CI green), tip `02af27c`, 6 commits across three
RED→GREEN cycles (initial build; a HIGH `cover_letter_chunks` PII-leak security fix; the
...
and gate-green, pending a PR**, on branch `feat/phase-7-evals-viewer` (off `main` @ `e910669`, tip
`92ca4ae`) — all three merge-blocking gates green (reviewer APPROVE, security PASS, ranking-evals PASS).
Phase 7 shipped the viewer only; the plan's Phase 7 evals-fixtures line item was already satisfied by 4a
(corpus) + 4c (live orchestrator wiring) — see [activity/phase-7-evals-viewer.md](activity/phase-7-evals-viewer.md)
...
**Live end-to-end eval — deferred, not built (ADR-013 §5).** The 4a/4c corpus has been proven against the
orchestrator called directly (4c) but never through the real HTTP→Postgres/Neo4j/Ollama pipeline (Phase
6's routes → persisted `shortlist_entries` rows, re-checked against `thresholds.toml`). This needs a
reachable host Ollama + `docker compose up`, which CI does not provide by design. Recorded as a documented
follow-up chore, not a gap in any currently-running gate.
...
`weights_from_settings`; full detail: [ADR-010](adr/010-shortlist-reverse-match-write-path.md) §3) but did
**not** touch `stages.py`/`orchestrator.py`'s scoring code (byte-unchanged) — the still-open
`jd.education.fields` human decision (ADR-009 §7) is therefore **still open**, unaffected by 4d.

**Phase 6 closed ADR-006 §4's `JobOut.blind_review` fail-open note** (`_row_to_jobout` now sets it
explicitly from the row on every construction path; full detail:
[ADR-012](adr/012-api-routes-auth-upload-scope.md) "Carry-forwards now CLOSED") and shipped the first code
path (`PATCH /jobs/{id}/status`) that ever transitions `jobs.status` — which makes ADR-010 §4's note about
`reverse_match_job`'s `allowed_job_ids` filter (`description_parsed IS NOT NULL`, not `status='open'`)
**partially relevant again**: a status route now exists, so scoping the filter to `status='open'` is no
longer structurally impossible, but Phase 6 did not itself change `matching_tasks.py`, so the filter is
...
triggers no writes). Phase 7's own new residual is the deferred live end-to-end eval, above.
