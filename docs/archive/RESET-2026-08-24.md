> **ARCHIVED 2026-08-27 — this document did its job.** It was written on
> 2026-08-24 against a product that had never had a user. It has since been
> demoed to the CIO and HR, approved by both, and **deployed to a dedicated box
> where four people are using it.** Its definition of done is met.
>
> Kept because the *diagnosis* is the reason `CLAUDE.md`'s Economy section
> exists, and because a repo that forgets why it stopped gold-plating will do it
> again. Read it as history, never as a queue. Live plan:
