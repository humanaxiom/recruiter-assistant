...
Read this first if you're resuming cold. It captures state, environment quirks, and the exact next step. The full plan is [docs/EXTRACTION_PLAN.md](../EXTRACTION_PLAN.md) — this file is the orientation layer.
...
[ADR-045](../adr/045-model-acceptance.md) records the architecture: the model is
...
[ADR-036](../adr/036-auditor-audit-log-viewer.md)'s D1 amendment.
...
- **D1 = C — ANSWERED, NOT IMPLEMENTED.** The audited reveal for withdrawal reasons is **the next piece of
  ...
  [docs/OPEN_DECISIONS.md](../OPEN_DECISIONS.md) until it lands — do not treat the answer as the delivery.
...
>
> Both found, fixed, and documented in [ADR-044](../adr/044-skill-family-classifier.md). See that ADR for the full story, including the non-determinism and why this matters for the 45.2% tail.
...
`already_running`. See **[ADR-043](../adr/043-shortlist-ranking-state.md)**.
...
It is also [A7](../ROADMAP.md) one layer further out than usual: the invariant was in a Jinja
...
Full detail in [ADR-041's addendum](../adr/041-sub-score-measurement-markers.md#addendum--three-siblings-now-marked-and-disclosed-2026-08-18). Implementation mirrors ADR-041 exactly, with one structural difference (same field names, not renamed) and one precedence rule (bar-not-stated wins for education when both markers are false).
...
1. **Get the pilot RUN.** Two *physical* human actions (run `quickstart.ps1` under `pwsh`; click through the
   live UI) plus **two letters** for the memos in [docs/OPEN_DECISIONS.md](../OPEN_DECISIONS.md). Highest-value item in the project, for a fourth session.
...
**The two blocked decisions now have memos: [docs/OPEN_DECISIONS.md](../OPEN_DECISIONS.md).** Options,
...
**The shape:** this is [A7](../ROADMAP.md) one level out from code. The invariant "skills come from two
...
> and a recommended default in **[docs/OPEN_DECISIONS.md](../OPEN_DECISIONS.md)**, per `CLAUDE.md` Economy
...
[ROADMAP A0.1](../ROADMAP.md)'s, not the explainer's — grepping the explainer for it finds nothing. The
...
Two ranking sub-scores fell back to a number byte-identical to a real measurement — [ADR-040](../adr/040-evidence-cliff-disclosure.md)'s
...
| 72 | **The auth boundary was OFF in the shipped config** | [034](../adr/034-auth-boundary-fails-open.md) |
| 73 | The ADR + doc pass #72 shipped without | — |
| 74 | **CSRF on all 12 browser write routes** (was 3) — A1 step (iv) | [035](../adr/035-csrf-on-every-browser-write-route.md) |
| 75 | **The auditor's access-record viewer** — Phase 1.4 | [036](../adr/036-auditor-audit-log-viewer.md) |
| 76 | `quickstart.ps1` could not be parsed by the PowerShell it claims to support | — |
| 77 | **Stage 3 fails closed on a non-LLM error** — A4 M1 | [037](../adr/037-stage3-fails-closed-on-non-llm-error.md) |
| 78 | **Gate the bait-below-strong ordering** the corpus only asserted in prose — A3's first move | [038](../adr/038-gate-the-bait-below-strong-ordering.md) |
| 79 | **Stage-1 recall searches the job's pool, not the whole DB** — A4 M2 | [039](../adr/039-stage1-recall-is-job-scoped.md) |
| 80 | **Disclose the evidence cliff** instead of a fabricated 0% — closes A4 | [040](../adr/040-evidence-cliff-disclosure.md) |
...
> [ADR-033](../adr/033-session-role-enforcement-on-writes.md) for full detail, including why ROADMAP
...
> unresolved. See [docs/ROADMAP.md](../ROADMAP.md) §A2 for the measured data and the revised plan.
>
...
> unchanged. Use [docs/ROADMAP.md](../ROADMAP.md) PART A for the full current state.
...
> Flask template renders its output and computes nothing itself. Full detail: **[ADR-031](../adr/031-why-this-rank-defense-pack.md)**.
>
...
[docs/activity/phase-4a-ranking-evals-corpus.md](../activity/phase-4a-ranking-evals-corpus.md); contract
detail: [docs/EXTRACTION_PLAN.md](../EXTRACTION_PLAN.md) ("4a hardening" subsections and "Current status
& next step").
...
Full write-up: [docs/activity/phase-3-ingest-parse.md](../activity/phase-3-ingest-parse.md); rationale: [docs/adr/007-*.md](../adr/007-phase3-ingest-parse-hardening.md).
...
[docs/activity/phase-4b-graph-projection.md](../activity/phase-4b-graph-projection.md); PII
architecture rationale: [docs/adr/008-skill-graph-pii-by-construction.md](../adr/008-skill-graph-pii-by-construction.md).
...
[docs/activity/phase-4c-matching-engine.md](../activity/phase-4c-matching-engine.md); decisions +
residuals: [docs/adr/009-matching-engine-port.md](../adr/009-matching-engine-port.md).
...
[docs/activity/phase-4d-shortlist-writepath.md](../activity/phase-4d-shortlist-writepath.md);
decisions + residuals: [ADR-010](../adr/010-shortlist-reverse-match-write-path.md).
...
[docs/activity/phase-5-persist-anonymize-export.md](../activity/phase-5-persist-anonymize-export.md);
decisions + residuals: [ADR-011](../adr/011-display-redaction-read-export-boundary.md).
...
write-up: [docs/activity/phase-6-api-routes.md](../activity/phase-6-api-routes.md); decisions +
residuals: [ADR-012](../adr/012-api-routes-auth-upload-scope.md).
...
[docs/activity/phase-7-evals-viewer.md](../activity/phase-7-evals-viewer.md);
decisions + residuals: [ADR-013](../adr/013-phase7-evals-viewer.md).
...
[ADR-013 §5](../adr/013-phase7-evals-viewer.md) and
[docs/activity/phase-7-evals-viewer.md](../activity/phase-7-evals-viewer.md)'s "Live end-to-end eval
...
Full detail: [ADR-014](../adr/014-workflow-ui.md).
...
>
> **🌟 NEXT-GEN "WOW" FEATURE MENU → [`docs/ROADMAP.md`](../ROADMAP.md).** Session wrapped 2026-08-04 for a
...
[ADR-022](../adr/022-uncited-evidence-quote-scrub.md).
...
(see "Queued next work" below). Full detail: [ADR-023](../adr/023-evidence-verifier-hardening.md); ADR-022
...
Full decision record: [ADR-026](../adr/026-resume-withdrawal-lifecycle.md) (see its "Built (FU-8,
...
- **Résumé lifecycle — candidate withdrawal + stale-résumé tracking (user request 2026-07-28, scoped in
  [ADR-026](../adr/026-resume-withdrawal-lifecycle.md)).** Two
...
[ADR-025](../adr/025-user-admin-roles.md); the superseded ADR-019 §10a default (`role='recruiter'` on
