...
```mermaid
flowchart LR
    U[Browser, off-LAN] -- HTTPS :8000 --> DNS[IONOS DNS sfuai.ca -> 173.180.31.84]
    DNS --> R[Telus router<br/>NAT: TCP 8000-8009 -> 192.168.1.80:8000-8009<br/>port 443 NOT forwarded]
...
Add a router NAT rule `TCP 443 -> 192.168.1.80:10443` (the existing
...
- `:29500` and `:29800` stay published on `0.0.0.0` (the nginx container
  ...
  `http://192.168.1.80:29800/docs` is readable by anything on the LAN.
