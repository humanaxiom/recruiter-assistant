...
**Operational**
- **In-app split of a combined Taleo export is the follow-on to the 2026-09-18 refusal** (SPONSOR_REQUIREMENTS_PLAN §2.0: server-side segmentation with a mandatory confirmation screen, because a mis-split attributes one person's experience to another). Blocked on measuring the segmentation prompt's budget at real concurrency (`scripts/model-check.sh`; no entry in `docs/model-profiles/` today) — `core/scripts/split_taleo_pdf.py` `_llm_segment`. Until then the upload refuses an unsplit export and points at `scripts/split-taleo.sh`.
- **NIT, security audit 2026-09-18** — `combined_export.looks_like_combined_export`'s false-negative list should also name "a small export (≤7 pages, 2 applicants) is never flagged" — added to the ADR-017 amendment text but worth a one-line pointer here too.
- **NIT, security audit 2026-09-18** — the upload route's `cover_text_map` (`src/api/routes/resumes.py`) is keyed by `basename_lower`, so two same-named cover-named files uploaded from different zip folders in one batch collapse to one map entry (pre-existing behaviour, not introduced by the F2/F3 fixes — `pair_applicants`'s `by_name` dict has the identical property).
- **NIT, security audit 2026-09-18** — `split_taleo_pdf.py`'s `_run_llm_mode` exit-code wiring (`clean and cover_only_count == 0`) is untested; `core/scripts` is outside the coverage gate by design (see the note atop the Makefile's gates target), so this would need a hand-run to verify.
...
- **`doctor.py`'s `neo4j.unprojected_jobs` misdiagnoses a zero-requirements job** (2026-09-15): a JD that parsed to no skills has no `REQUIRES` edges by construction, and the finding's remedy ("check the outbox drainer and re-parse") sends the operator after a projection fault that does not exist. The check should join the Postgres side and say "this JD has no requirements" instead. It fires on the pilot's own job today.
- **An auditor's shortlist page view now writes a `read_job` audit row it did not ask for** (2026-09-15): `job_shortlist` fetches the job to decide the zero-requirements banner, and `GET /jobs/{id}` is `log_auditor_read`-audited. Accepted as audit-trail inflation, not a leak; skip the fetch for non-writers if it ever matters.
...
- **Residuals from the 2026-09-23 security audit of the résumé re-parse** (all low, recorded not fixed): (a) the reset nulls `parsed` BEFORE the retry runs, so a degraded row whose retry then fails lands `failed` with no parse output — acceptable because a degraded row is already excluded from ranking, but there is no undo (`resume_service.reset_for_reparse`); (b) each click resets `reconcile_attempts` to 0 and nothing in the app rate-limits, so a writer can spend GPU passes click-by-click — bounded by the in-flight 409 and a human click per cycle, same shape as `POST /jobs/{id}/reparse`; (c) neither re-parse route writes an `audit_log` row, unlike reveal/withdraw; (d) three writer sites in `worker/resume_tasks.py` still interpolate a raw `{exc}` into the now-rendered `failure_reason` (blob fetch, text extraction, embedding) — capped at 1000 chars, PII-free by the extractors' own error text today, but not pinned by a test.
...
- **NIT, 2026-09-17 review** — `job_detail.html` uses both `job_id|default(job.id)` and the bare `job.id` for the same value in different forms on the page (e.g. [job_detail.html:228](../core/frontend/templates/job_detail.html#L228) vs the `url_for` calls around it); harmless (both resolve to the same string) but worth picking one spelling next time the template is touched.
- **NIT, 2026-09-17 review** — the role-filtered `GET /users?role=...` list (`user_service.list_users`, [core/src/services/user_service.py](../core/src/services/user_service.py)) includes deactivated (`active = false`) users alongside active ones; the hiring-manager-picker dropdown it feeds can therefore offer a deactivated account. Not acted on: the unfiltered admin listing has the same property by design (admins need to see deactivated rows to reactivate them), and narrowing only the filtered read would make the two reads inconsistent for no clear benefit yet.
- **NIT, 2026-09-17 review** — several new/renamed tests in this branch (`test_route_users.py`, `test_route_shortlist.py`, `test_shortlist_waits_for_projection.py`) use long, sentence-shaped names (e.g. `test_get_users_filtered_by_role_admin_403s_for_a_recruiter_session`); consistent with this repo's existing convention elsewhere in the same files, not a new pattern, but flagged as a style nit rather than silently normalised.
