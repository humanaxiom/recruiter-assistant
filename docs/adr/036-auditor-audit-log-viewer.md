...
`audit_log.details` is the only free-text column in the table. Two writers populate it:
...
| `withdraw_resume` | `{"reason": <operator prose>}` | **Withheld.** Arbitrary text a human typed about a specific, named candidate. |
...
surface that can be edited from the browser is not an audit surface. Unattributable service events are
...
- **Whether an auditor should see withdrawal reasons is UNRESOLVED and needs a human.** It is plausibly
  within an auditor's remit and plausibly a PIPEDA/FIPPA problem. This slice makes the safe choice, says so
  in the UI, and records the question rather than answering it by implementation — which is how ADR-033's
  residual became ADR-034.
