...
(``phase3-source-dossier.md`` §9). Phase 3 ports exactly four prompt
pairs: ``jd_extract_v1``, ``resume_core_v1``, ``resume_skills_v2``,
``cover_letter_v1``. ``shortlist_evidence_v1`` is Phase 4 (evidence
extraction) and must NOT be present yet.
...
def test_list_prompts_returns_exactly_the_four_ported_names() -> None:
...
def test_shortlist_evidence_v1_is_not_ported_it_is_phase_4() -> None:
    assert "shortlist_evidence_v1" not in list_prompts()
