...
from src.agents.security import SecurityReport
from src.gates.review_loop import LoopOutcome
...
def _coder_plan() -> Plan:
    return make_plan(
        [
            Subtask(id="t1", agent="tester", task="tests", depends_on=[]),
            Subtask(id="t2", agent="coder", task="implement", depends_on=["t1"]),
            Subtask(id="t3", agent="reviewer", task="review", depends_on=["t2"]),
        ]
    )


@pytest.mark.asyncio
async def test_coder_loop_success(orch: Orchestrator) -> None:
    orch._planner.run = AsyncMock(return_value=ok_output("planner", _coder_plan()))  # type: ignore[method-assign]
    orch._agents["tester"].run = AsyncMock(return_value=ok_output("tester", ["f"]))  # type: ignore[method-assign]
    orch._agents["reviewer"].run = AsyncMock(  # type: ignore[method-assign]
        return_value=ok_output("reviewer", Review(approved=True))
    )
    orch._git_diff = AsyncMock(return_value="")  # type: ignore[method-assign]
    outcome = LoopOutcome(True, 1, GateSuiteResult([]), False, [])
    with patch("src.agents.orchestrator.ReviewLoop") as review_loop:
        review_loop.return_value.run = AsyncMock(return_value=outcome)
        result = await orch.run("T", "spec")
    assert result.success is True
    assert result.loop_outcome is outcome


@pytest.mark.asyncio
async def test_coder_escalation_blocks(orch: Orchestrator) -> None:
    orch._planner.run = AsyncMock(return_value=ok_output("planner", _coder_plan()))  # type: ignore[method-assign]
    orch._agents["tester"].run = AsyncMock(return_value=ok_output("tester", ["f"]))  # type: ignore[method-assign]
    failed = GateSuiteResult([GateResult("mypy", GateStatus.RED, "err", 1)])
    outcome = LoopOutcome(False, 5, failed, True, [])
    with patch("src.agents.orchestrator.ReviewLoop") as review_loop:
        review_loop.return_value.run = AsyncMock(return_value=outcome)
        result = await orch.run("T", "spec")
    assert result.success is False
    assert result.blocked_by == "coder"


@pytest.mark.asyncio
async def test_dependency_cycle_blocks_as_planner(orch: Orchestrator) -> None:
    cyclic = make_plan(
        [
            Subtask(id="a", agent="tester", task="", depends_on=["b"]),
            Subtask(id="b", agent="reviewer", task="", depends_on=["a"]),
    ...
    orch._planner.run = AsyncMock(return_value=ok_output("planner", cyclic))  # type: ignore[method-assign]
    result = await orch.run("T", "spec")
    assert result.success is False
    assert result.blocked_by == "planner"
...
@pytest.mark.asyncio
async def test_subtask_exception_is_structured(orch: Orchestrator) -> None:
    plan = make_plan([Subtask(id="t1", agent="tester", task="t", depends_on=[])])
    orch._planner.run = AsyncMock(return_value=ok_output("planner", plan))  # type: ignore[method-assign]
    orch._agents["tester"].run = AsyncMock(side_effect=RuntimeError("boom"))  # type: ignore[method-assign]
    result = await orch.run("T", "spec")
    assert result.success is False
    assert result.blocked_by == "tester"
    assert "boom" in (result.outputs["t1"].error or "")


@pytest.mark.asyncio
async def test_security_failure_blocks(orch: Orchestrator) -> None:
    plan = make_plan(
        [
            Subtask(id="t1", agent="tester", task="tests", depends_on=[]),
            Subtask(id="t2", agent="security", task="audit", depends_on=["t1"]),
        ]
    )
    orch._planner.run = AsyncMock(return_value=ok_output("planner", plan))  # type: ignore[method-assign]
    orch._agents["tester"].run = AsyncMock(return_value=ok_output("tester", ["f"]))  # type: ignore[method-assign]
    orch._agents["security"].run = AsyncMock(  # type: ignore[method-assign]
        return_value=ok_output("security", SecurityReport(passed=False))
    )
    orch._git_diff = AsyncMock(return_value="")  # type: ignore[method-assign]
    result = await orch.run("T", "spec")
    assert result.success is False
