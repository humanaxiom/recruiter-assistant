...
**FU-7 (scoped by ADR-021, 2026-07-20) is built in two slices; one decision remains open:**
...
| **§2 + §6 — fail-closed ranking + empty-content detection** | branch `feat/fu7-fail-closed-ranking` | [029](docs/adr/029-fail-closed-ranking-fu7.md) | Fails closed on **both** an LLM outage and invalid/empty output during forward-shortlist ranking (widened by human decision beyond §2's literal Mode-B-only scope); a dedicated `jobs.shortlist_state` column trio + bounded arq retry replace the prior silent zero-score; empty-`content` is now detected and diagnosed on both LLM client code paths |

**Still open (ADR-021 decisions 1 and 4, not yet scheduled):** an ordered multi-provider chain with per-provider circuit breakers and failover on availability errors (decision 1); `degraded`/`degradation_reason` visibility on a résumé whose skills extraction failed, and excluding degraded résumés from ranking (decision 4).
