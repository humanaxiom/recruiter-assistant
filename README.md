...
- **Blind review is opt-in per job** — a recruiter switches it on per requisition; when on, candidate name / email / phone are redacted in the viewer and exports, and reveal is opt-in and audited (decision 4, reversed 2026-09-09). Embeddings are scrubbed of candidate PII **unconditionally**, on every job, whether or not blind review is on — blind review only governs what a viewer/export displays, never what gets embedded.
