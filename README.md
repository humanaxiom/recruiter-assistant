...
**Five further features are merged to LOCAL `main` and running, but NOT yet pushed to `origin`** — the org's GitHub Actions billing limit is exhausted (jobs fail at "Set up job"), so since FU-5 the work has been integrated and verified locally (`./scripts/verify.sh all` green) rather than through CI. A human needs to clear the billing block, then `git push origin main` and reconcile the open PRs #30/#31. Ordered oldest-first:

| Feature | local commit / PR | ADR | What it added |
|---|---|---|---|
| **Live-app bug fixes** | `202c9ac` (PR #30) | — | zip-upload empty-file 422, résumé-upload 404, slow-job progress honesty, duplicate-run dedupe |
| **FU-6 — Per-job assignment + row-level scoping** | `1dfa590` (PR #31) | [020](docs/adr/020-per-job-assignment-scoping.md) | A `job_assignees` table so hiring managers see only their assigned jobs; scoping enforced off the CAS **session** role in SQL, unassigned reads 404 not 403; auditor read-logging; the `/my/jobs` scoped viewer default |
